...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoibWFya29zdGVyayIsImEiOiJjbDFkdXg3NTEwbGJ6M2RwNDA1N3NweDZjIn0.eJmGz8UqExuToCDlrbP19w';
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // Add pop-up with information about the marker (tour)

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //Extend the map bounds to include the location
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            if (res.data.status == 'success') {
              //location.reload(true); //true == forces a server reload not from the cach
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? '/app/v1/users/updateMyPassword' : '/app/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: url,
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', "".concat(type.toUpperCase(), " was updated successfully"));
            }
...
var stripe = Stripe('pk_test_51Kk1Q6Dv16OiKg9PkWhEhwaz848ttP2mddXgGgR1DRzwEN82iiQLeFZ5IIntrDDiCuADnK8DZZa3AyYwFq4gdsDl001dzVTH2e');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return (0, _axios.default)("/app/v1/bookings/checkout-session/".concat(tourId));
